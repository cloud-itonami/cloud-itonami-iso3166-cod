  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kin Fabrication SARL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kin Fabrication SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest preference-tier-mismatch-is-held-and-unoverridable
  (testing "Loi n°10/010 Art. 37 preference-tier mismatch (eng-4 declares foreign-congolese-economic-activity? = tier 5, but claims tier 2) -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:preference-tier-mismatch} (-> (store/ledger db) last :basis)))
